...
import {
    ...
    segmentIntersection,
    segmentOverlapType,
    segmentTouchesRectInterior,
    toSegments
} from "./helpers/bpmnXmlTestUtils.js";
...
test("generateBPMN renders all activities as standard tasks plus timer and annotations", () => {
    const process = {
        ...
        steps: [
            ...
            },
            {
                ...
                label: "E-Mail senden",
                role: "Einkauf",
                documentation: "Absage mit Vorlage",
                email: {
                    to: "kunde@example.com",
                    subject: "Absage",
                    template: "tpl_absage"
                },
                boundaryTimers: [{ label: "Warten", target: "step_3", interrupting: false, duration: "PT24H" }],
                next: ["step_3"]
            },
            { id: "step_3", type: "end", label: "Ende", role: "GF" }
        ]
    };
    ...
    assert.ok(/<bpmn:boundaryEvent[^>]+attachedToRef="step_2"/.test(xml));
    assert.ok(xml.includes("<bpmn:timerEventDefinition>"));
    assert.ok(xml.includes("PT24H"));
    assert.ok(xml.includes('cancelActivity="false"'));
    assert.ok(xml.includes('<bpmn:textAnnotation id="ann_freigabe"'));
    assert.ok(xml.includes("<bpmn:association id=\"Association_ann_freigabe\""));
});
